...
def do_train(args, model, train_dataloader, save_dir="./out"):
    ...
    print(first_batch["input_ids"][:10])
    print(first_batch["labels"][:20])
...
def create_augmented_dataloader(args, dataset):
    ...
    augmented_subset = train_dataset.select(sampled_indices)
    
    transformed_dataset = augmented_subset.map(custom_transform, load_from_cache_file=False)
    
    from datasets import concatenate_datasets
    combined_dataset = concatenate_datasets([train_dataset, transformed_dataset])
